...
class SKPagingScrollView: UIScrollView {
    ...
    func deleteImage() {
        // index equals 0 because when we slide between photos delete button is hidden and user cannot to touch on
        // delete button. And visible pages number equals 0
        if numberOfPhotos > 0 {
    ...
    func pageDisplayedAtIndex(_ index: Int) -> SKZoomingScrollView? {
        return visiblePages.first(where: { $0.tag - pageIndexTagOffset == index })
    }
