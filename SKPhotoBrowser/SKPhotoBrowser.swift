...
extension NSNotification.Name {
    public static let SKPhotoLoadingDidEnd: NSNotification.Name =
            NSNotification.Name(rawValue: "photoLoadingDidEndNotification")
}
...
open class SKPhotoBrowser: UIViewController {
    ...
    fileprivate lazy var pagingScrollView: SKPagingScrollView = SKPagingScrollView(frame: self.view.frame,
                                                                                   browser: self)
    ...
    func setup() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               name: .SKPhotoLoadingDidEnd, object: nil)
    }
...
public extension SKPhotoBrowser {
    ...
    func hideControlsAfterDelay() {
        ...
        cancelControlHiding()
        // start
        controlVisibilityTimer = Timer.scheduledTimer(timeInterval: 4.0, target: self,
                                                      selector: #selector(SKPhotoBrowser.hideControls(_:)),
                                                      userInfo: nil, repeats: false)
    }

    func hideControls(animated: Bool = true) {
        setControlsHidden(true, animated: animated, permanent: false)
    }
...
internal extension SKPhotoBrowser {
    ...
    @objc func actionButtonPressed(ignoreAndShare: Bool) {
        ...
        if let titles = SKPhotoBrowserOptions.actionButtonTitles {
            ...
            actionSheetController.addAction(UIAlertAction(title: cancelTitle, style: .cancel, handler: nil))
...
private extension SKPhotoBrowser {
    func configureAppearance() {
        ...
        backgroundView = UIView(frame: CGRect(x: 0, y: 0, width: SKMesurement.screenWidth,
                                              height: SKMesurement.screenHeight))
    ...
    func setControlsHidden(_ hidden: Bool, animated: Bool, permanent: Bool) {
        ...
        UIView.animate(withDuration: 0.35,
            animations: { () -> Void in
                ...
                self.toolbar.frame = hidden ? self.frameForToolbarHideAtOrientation()
                                            : self.frameForToolbarAtOrientation()
