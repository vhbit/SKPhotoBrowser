...
typealias SKPhotoAssetCompletion = (UIImage?, PHImageRequestID?) -> Void

class FromCameraRollViewController: UIViewController, SKPhotoBrowserDelegate, UICollectionViewDataSource,
...
//            browser.displayAction = false
        ...
        var fetchedImages: [UIImage] = Array(repeating: UIImage(), count: assets.count)
        var fetched = 0

        assets.forEach { asset in

            _ = requestImageForAsset(asset, options:bigRequestOptions, completion: { [weak self] image, _ in
    ...
    fileprivate func requestImageForAsset(_ asset: PHAsset, options: PHImageRequestOptions,
                                          completion: @escaping SKPhotoAssetCompletion) -> PHImageRequestID {
        ...
        if asset.representsBurst {
            ...
            let result = imageManager.requestImage(for: asset, targetSize: targetSize, contentMode: .aspectFill,
